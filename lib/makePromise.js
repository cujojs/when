...
define(function() {
	...
	function makePromise(environment) {
		...
		Promise.prototype.then = function(onFulfilled, onRejected, onProgress) {
			...
			tasks.enqueue(new WhenTask(onFulfilled, onRejected, onProgress, this, p));
			return p;
		};
		 ...
		 * @param {*} x
		 */
		function doResolvePromise(p, x) {
			p._handler = p._handler.join(getHandlerChecked(p, x));
		}

		/**
		 * Reject p with x verbatim
		 * @param {Promise} p
		 * @param {*} x
		 */
		function doRejectPromise(p, x) {
			p._handler = p._handler.join(new RejectedHandler(x));
		}
		...
		function getHandlerChecked(p, x) {
			return p === x
		...
		/**
		 * Task that awaits a from promise and resolves a to promise
		 * after transforming the outcome
		 * @private
		 * @constructor
		 */
		function WhenTask(f, r, u, from, to) {
			this.f = f;
			this.r = r;
			this.u = u;
			this.from = from;
		...
		WhenTask.prototype.run = function() {
			if(this.from._handler instanceof FulfilledHandler) {
				joinNext(this.f, this.from._handler, this.to);
			} else if(this.from._handler instanceof RejectedHandler) {
				joinNext(this.r, this.from._handler, this.to);
			} else {
				whenNext(this.f, this.r, this.u, this.from._handler, this.to);
			}
		};

		function joinNext(f, from, to) {
			var toHandler;
			try {
				toHandler = typeof f === 'function'
					? getHandlerChecked(to, f(from.value)) : from;
			} catch(e) {
				toHandler = new RejectedHandler(e);
			}
			to._handler = to._handler.join(toHandler);
		}

		function whenNext(f, r, u, from, to) {
			from.when(_resolve, notify, f, r, u);

			function _resolve(x) {
				doResolvePromise(to, x);
			}
		...
		AssimilateTask.prototype.run = function() {
			...
			try {
				call(this.untrustedThen, this.thenable, resolve, _reject, this.notify);
			} catch (e) {
				_reject(e);
			}
