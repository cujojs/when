	...
	return function makePromise(environment) {
		...
		function all(promises) {
			...
			function resolveOne(resolver, results, handler, i) {
				handler.visit(resolver, function(x) {
					results[i] = x;
					if(--pending === 0) {
		...
		function next(f, h, receiver, resolver) {
			Promise.enterContext(h);

			var x = typeof f === 'function'
				? tryCatchReject(f, h.value, receiver)
				: h;
		...
		Handler.prototype.when
			...
			= Handler.prototype.map
			= Handler.prototype.catchError
		...
		Handler.prototype.chain = function(to, c, fulfilled, rejected, progress) {
			this.when({
				resolver: to,
				receiver: c,
				fulfilled: fulfilled,
		...
		Handler.prototype.visit = function(c, fulfilled, rejected, progress) {
			this.when({
				resolver: failIfRejected,
				receiver: c,
				fulfilled: fulfilled,
				rejected: rejected,
				progress: progress
		...
		Handler.prototype.fold = function(f, z, to) {
			var c = this.receiver;
			this.visit(to, function (x) {
				getHandler(z).visit(this, function (z) {
					this.become(tryCatchReject2(f, z, x, c));
				}, this.reject);
			}, to.reject, to.notify);
		};
		...
		DeferredHandler.prototype.map = function(f, c, to) {
			this.chain(to, c, f);
		};

		DeferredHandler.prototype.catchError = function(f, c, to) {
			this.chain(to, c, void 0, f);
		};
		...
		DeferredHandler.prototype.fail = function(context) {
		...
		FulfilledHandler.prototype.map = function(f, c, to) {
			next(f, this, c, to);
		};

		FulfilledHandler.prototype.catchError = function(f, c, to) {
			to.become(this);
		};

		FulfilledHandler.prototype.when = function(cont) {
			this.map(cont.fulfilled, cont.receiver, cont.resolver);
		};
		...
		RejectedHandler.prototype.map = function(f, c, to) {
			to.become(this);
		};

		RejectedHandler.prototype.catchError = function(f, c, to) {
		...
		RejectedHandler.prototype.when = function(cont) {
			this.catchError(cont.rejected, cont.receiver, cont.resolver);
		};
		...
		RejectedHandler.prototype.fail = function(context) {
			Promise.onFatalRejection(this.value, context === void 0 ? this.context : context);
		};
		...
		ProgressTask.prototype._notify = function(cont) {
			var x = typeof cont.progress === 'function'
				? tryCatchReturn(cont.progress, this.value, cont.receiver)
				: this.value;

			cont.resolver.notify(x);
		};
		...
		function tryCatchReject(f, x, thisArg) {
			try {
				return getHandler(f.call(thisArg, x));
			} catch(e) {
				return new RejectedHandler(e);
			}
		}
		...
		function tryCatchReject2(f, x, y, thisArg) {
			try {
				return getHandler(f.call(thisArg, x, y));
			} catch(e) {
				return new RejectedHandler(e);
			}
		}
