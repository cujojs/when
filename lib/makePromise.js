	...
	function makePromise(environment) {
		...
		function Promise(resolver) {
			...
			function promiseResolve (x) {
				self._handler = self._handler.resolve(self, x);
			}
			/**
			...
			function promiseReject (reason) {
				self._handler = self._handler.reject(reason);
			}
			/**
		...
		FollowingHandler.prototype.resolve
			...
			= DeferredHandler.prototype.resolve = function(p, x) {
			return this.join(getHandlerChecked(p, x));
		};
