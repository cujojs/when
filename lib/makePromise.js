...
define(function() {
	...
	return function makePromise(environment) {
		...
		Promise.prototype.then = function(onFulfilled, onRejected, onProgress) {
			...
			// Tracing for thenable chain.
			to._addTrace(from.trace);
		...
		/**
		 * Private function to trace any spawned promise.
		 * @private
		 * @param {Promise} The new promise which is considered as spawned by the current.
		 *  the returned promise.
		 */
		Promise.prototype._spawn = function(promise){
			var trace = this._handler.trace;
			if(trace && promise._handler)
			  promise._handler._addTrace(trace);
		};
		...
		DeferredHandler.prototype._join = function(handler) {
			...
			if(this._isMonitored()) {
				// Join with the other deferred handler.
				if (handler instanceof DeferredHandler) {
					var joint = handler.trace;

					var head = joint;
					while(head.parent) {
						head = head.parent;
					}
					// Make the head of joint tracing me.
					head.parent = this.trace.clone();
					// Let the joint be my trace.
					this.trace.merge(joint);
				}
				else
					handler._addTrace(this.trace);
			}
		};
