...
define(function() {
	...
	return function makePromise(environment) {
		...
		Promise.prototype._beget = function() {
			...
			var h = this._handler;
			return configurePromise(new DeferredHandler(h.receiver, h.join().context), p);
		};
		...
		RejectedHandler.prototype._reportTrace = function(context) {
			this._env.promiseMonitor.addTrace(this.id, this.value, this.context, context);
		};

		RejectedHandler.prototype._removeTrace = function() {
			this._env.promiseMonitor.removeTrace(this.id);
		};

		RejectedHandler.prototype._fatal = function() {
			this._env.promiseMonitor.fatal(this.value, this.context);
		};

		// Errors and singletons

		foreverPendingHandler = new Handler();
		foreverPendingPromise = promiseFromHandler(foreverPendingHandler);
