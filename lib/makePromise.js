...
define(function() {
	...
	function makePromise(environment) {
		...
		function resolvePromise(p, x) {
			if(p === x) {
				p._handler = p._handler.reject(new TypeError());
			}
			p._handler = p._handler.resolve(x);
		}

		/**
		 * Reject promise with x verbatim
		 * @param {Promise} p
		 * @param {*} x reason with which to reject p
		 */
		function rejectPromise(p, x) {
			p._handler = p._handler.reject(x);
		}
		...
		DeferredHandler.prototype.resolve = function(x) {
			return this._join(getHandler(x));
		};

		DeferredHandler.prototype.reject = function(x) {
			return this._join(new RejectedHandler(x));
		};
		...
		FollowingHandler.prototype.resolve = FollowingHandler.prototype.reject
			= FulfilledHandler.prototype.resolve = FulfilledHandler.prototype.reject
			= RejectedHandler.prototype.resolve = RejectedHandler.prototype.reject
