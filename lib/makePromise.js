...
define(function() {
	...
	return function makePromise(environment) {
		...
		Promise.prototype.then = function(onFulfilled, onRejected, onProgress) {
			...
			from.when(to, from.receiver, onFulfilled, onRejected, onProgress);
		...
		DeferredHandler.prototype.when = function(handler, receiver, f, r, u) {
			if (this.resolved) {
				this.traverse().when(handler, receiver, f, r, u);
			} else {
				this.consumers.push(new ForwardingTask(handler, receiver, f, r, u));
			}
		};
		...
		FollowingHandler.prototype.when = function(handler, receiver, f, r, u) {
			...
			this.traverse().when(handler, this.receiver === void 0 ? receiver : this.receiver, f, r, u);
		};
		...
		FulfilledHandler.prototype.when = function(handler, receiver, f) {
			tasks.enqueue(new FulfillTask(handler, receiver, f, this.value));
		};
		...
		RejectedHandler.prototype.when = function(handler, receiver, _, r) {
			tasks.enqueue(new RejectTask(handler, receiver, r, this.value));
		};
		 ...
		 * @constructor
		 */
		function FulfillTask(handler, receiver, map, value) {
		...
		FulfillTask.prototype.run = function() {
			if(typeof this.map === 'function') {
				this.handler.resolve(tryCatchReject(this.map, this.value, this.receiver));
			} else {
				this.handler.resolve(this.value);
			}
		};
		 ...
		 * @constructor
		 */
		function RejectTask(handler, receiver, map, value) {
			this.handler = handler;
			this.receiver = receiver;
			this.map = map;
			this.value = value;
		...
		RejectTask.prototype.run = function() {
			if(typeof this.map === 'function') {
				...
				this.handler.reject(this.value);
			}
		};
		...
		ProgressTask.prototype.when = function(handler, receiver, __, ___, u) {
			try {
				handler.notify(typeof u === 'function'
