	...
	return function makePromise(environment) {
		...
		function next(f, h, receiver, resolver) {
			...
			var x = typeof f === 'function'
				? getHandler(tryCatchReject(f, h.value, receiver))
				: h;
		...
		Handler.prototype.visit = function(c, fulfilled, rejected, progress) {
			this.chain(done, c, fulfilled, rejected, progress);
		};
		...
		Handler.prototype.map = noop;
		Handler.prototype.catchError = noop;

		function Done() {}
		inherit(Handler, Done);
		Done.prototype.become = function(h) {
			h.fail();
		};
		var done = new Done();
		...
		DeferredHandler.prototype.join = function() {
			// TODO: Check self-cycles while collapsing
			if (this.resolved) {
				var h = this;
				while(h.handler !== void 0) {
					h = h.handler;
		...
		DeferredHandler.prototype.map = function(f, to) {
			this.when({
				resolver: to,
				receiver: this.receiver,
				fulfilled: f, rejected: void 0, progress: void 0
			});
		};
		...
		FulfilledHandler.prototype.when = function(cont) {
			next(cont.fulfilled, this, cont.receiver, cont.resolver);
		};
		...
		RejectedHandler.prototype.when = function(cont) {
			this._unreport();
			next(cont.rejected, this, cont.receiver, cont.resolver);
		};
		...
		RejectedHandler.prototype._fatal = function(context) {
			...
			RejectedHandler.prototype.fail = function() {
				Promise.onFatalRejection(this.value, this.context);
			};
