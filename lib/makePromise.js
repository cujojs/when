...
define(function() {
	...
	return function makePromise(environment) {
		...
		function all(promises) {
			...
			function resolveOne(resolver, results, handler, i) {
				handler.visit(resolver, function(x) {
					results[i] = x;
					if(--pending === 0) {
						this.become(new Fulfilled(results));
					}
		...
		Pending.prototype.map = function(f, c, to) {
			this.chain(to, c, f);
		};

		Pending.prototype.catchError = function(f, c, to) {
		...
		Delegating.prototype.map = function(f, c, to) {
			this.join().map(f, c, to);
		};

		Delegating.prototype.catchError = function(f, c, to) {
			this.join().catchError(f, c, to);
		};
		...
		Fulfilled.prototype.map = function(f, c, to) {
			runInContext(f, this, c, to);
		};

		Fulfilled.prototype.fold = function(f, z, c, to) {
			var x = this.value;
			getHandler(z).visit(to, function(z) {
				this.become(tryCatchReject2(f, z, x, c));
			}, to.reject, to.notify);
		};

		Fulfilled.prototype.catchError = function(f, c, to) {
			to.become(this);
		};
		...
		Rejected.prototype.map = function(f, c, to) {
			to.become(this);
		};

		Rejected.prototype.catchError = function(f, c, to) {
		...
		function runInContext(f, h, receiver, to) {
			if(typeof f !== 'function') {
				return to.become(h);
			}

			Promise.enterContext(h);
			var x = tryCatchReject(f, h.value, receiver);
			Promise.exitContext();
